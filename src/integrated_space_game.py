import sys
import random
from PyQt6.QtWidgets import QApplication, QMainWindow, QGraphicsView, QGraphicsScene, QGraphicsItem, QWidget, QVBoxLayout, QHBoxLayout
from PyQt6.QtCore import Qt, QTimer, QRectF, QRect
from PyQt6.QtGui import QPainter, QColor, QBrush, QMovie, QPixmap

class OverlayMedia(QWidget):
...
class GameWindow(QMainWindow):
    def __init__(self):
        ...
        self.overlay = OverlayMedia(800, 600, gif_infos=[("player.gif", 380, 200)])
    ...
    def move_player(self, delta_x):
        for gif_info in self.overlay.gif_infos:
            path, x, y = gif_info
            new_x = max(0, min(self.width() - 40, x + delta_x))
            self.overlay.move_gif(self.overlay.gif_infos.index(gif_info), (new_x, y))
    ...
    def update_game_objects(self):
        ...
        player_rect = QRectF(self.overlay.gif_infos[0][1], self.overlay.gif_infos[0][2], self.overlay.gifs[0].currentPixmap().width(), self.overlay.gifs[0].currentPixmap().height())
    ...
    def shoot_bullet(self):
        bullet = Bullet(self.overlay.gif_infos[0][1] + 18, self.overlay.gif_infos[0][2])
